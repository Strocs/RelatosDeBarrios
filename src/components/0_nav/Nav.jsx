	...
	return (
		<nav className={`nav ${navStyle.height}`}>
			<div className={`nav__container ${navStyle.background}`}>
				...
				{mobile && <HamburgerBtn onClick={toggleMenu} menuState={showMenu} />}

				{showMenu && mobile && <NavMenu list={menu} onClick={toggleMenu} menuState={showMenu} />}
				{desktop && <NavMenu list={menu} onClick={toggleMenu} menuState={true} />}
			</div>
		</nav>
	)
}
